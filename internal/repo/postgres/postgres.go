...
func (r *OrderRepo) CreateOrder(
	...
	orderQuery, args, err := goqu.Insert("orders").Prepared(true).Rows(order).ToSQL()
	if err != nil {
		return e.Wrap(op, err)
	}
	...
	deliveryQuery, args, err := goqu.Insert("delivery").Prepared(true).Rows(delivery).ToSQL()
	if err != nil {
		return e.Wrap(op, err)
	}
	...
	paymentQuery, args, err := goqu.Insert("payment").Prepared(true).Rows(payment).ToSQL()
	if err != nil {
		return e.Wrap(op, err)
	}
	...
	itemsQuery, args, err := goqu.Insert("items").Prepared(true).Rows(items).ToSQL()
	if err != nil {
		return e.Wrap(op, err)
	}
...
func (r *OrderRepo) GetOrder(ctx context.Context, ID string) (domain.FullOrder, error) {
	...
	sql, args, err := goqu.From("orders").
		LeftJoin(goqu.T("delivery"), goqu.On(goqu.Ex{"orders.id": goqu.I("delivery.order_id")})).
		LeftJoin(goqu.T("payment"), goqu.On(goqu.Ex{"orders.id": goqu.I("payment.order_id")})).
		Select(
			// orders
			goqu.I("orders.id"),
		...
		//LeftJoin(goqu.T("items"), goqu.On(goqu.Ex{"orders.id": goqu.I("items.order_id")})).
